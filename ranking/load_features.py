...
from utils import BASEPATH_FEATURES, FEATURE_SET_NAME, ES_HOST,ES_User,ES_Passw
...
def load_features(feature_set_name: str):
    ...
    head = {'Content-Type': 'application/json'}
    resp = requests.post(full_path, data=json.dumps(feature_set), headers=head,auth=HTTPBasicAuth(ES_User,ES_Passw),verify=False)
...
def init_default_store():
    ...
    resp = requests.delete(path,auth=HTTPBasicAuth(ES_User,ES_Passw),verify=False)
    Logger.logger.info("%s" % resp.status_code)
    Logger.logger.info("PUT %s" % path)
    resp = requests.put(path,auth=HTTPBasicAuth(ES_User,ES_Passw),verify=False)
    Logger.logger.info("%s" % resp.status_code)
